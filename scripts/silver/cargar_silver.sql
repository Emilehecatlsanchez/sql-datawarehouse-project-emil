...
BEGIN
    ...
    SELECT 
        ...
        END AS prd_line,
        prd_start_dt,
        -- the new end_date = start data of next record - 1 day (in the case where end_dt is smaller than start date)
        DATEADD(day, -1, LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)) AS prd_end_dt  -- to substract a day
